...
def gen_makefile(cfg_filename, make_filename):
    ...
    make.rule('all', ['all_tikz', 'all_pdf', 'all_tikz_pdf', 'all_tikz_svg', 'all_pdf_svg'], '', phony=True)
    ...
    make.rule('clean', ['clean_tikz', 'clean_tikz_pdf', 'clean_tikz_svg',
                        'clean_pdf', 'clean_symlinks'], '', phony=True)
    make.rule('clean_tikz', '', [
              f'rm -f {" ".join(all_targets["tikz"])}', 'rm -f *_ras1.png'], phony=True)
    make.rule('clean_tikz_pdf', '', [
              f'rm -f {" ".join(all_targets["tikz_pdf"])}'], phony=True)
    make.rule('clean_tikz_svg', '', [
              f'rm -f {" ".join(all_targets["tikz_svg"])}'], phony=True)
    make.rule('clean_pdf', '',
              f'rm -f {" ".join(all_targets["pdf"])}', phony=True)
    make.rule('clean_symlinks', '',
              f'rm -f {" ".join(["config", "figure"])}', phony=True)
